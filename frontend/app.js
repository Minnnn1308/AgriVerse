// ================================================================
// AgriVerse v0.0.8.0 - JavaScript Revolution
// Mobile-first, farmer-friendly, online-ready experience
// ================================================================

const isProd = window.location.hostname !== '127.0.0.1' && window.location.hostname !== 'localhost';
const BACKEND_URL = isProd ? "https://agriverse-backend.onrender.com" : "http://127.0.0.1:8002";

const API_BASE = `${BACKEND_URL}/gamification`;
const WEATHER_API = `${BACKEND_URL}/weather`;
let USER_ID = localStorage.getItem('agriverse_user_id') || "J-007";
let FARM_ID = localStorage.getItem('agriverse_farm_id') || "FARM_001";
const APP_VERSION = "0.0.8.0";
...
function showToast(message, type = 'info') {
    ...
    toast.innerHTML = `<div class="toast-message">${String(message)}</div>`;
...
window.alert = function(message) {
    ...
    const type = /❌|lỗi|thất bại/i.test(text) ? 'error' : /🎉|✨|✅|thành công|chào mừng/i.test(text) ? 'success' : /⚠️|cảnh báo/i.test(text) ? 'warning' : 'info';
    showToast(text, type);
    return undefined;
};
        ...
        if (progress >= 100) {
            ...
            setTimeout(() => {
                ...
                if (!token) {
                    ...
                    // Load dynamic IDs
                    USER_ID = localStorage.getItem('agriverse_user_id') || "J-007";
                    FARM_ID = localStorage.getItem('agriverse_farm_id') || "FARM_001";
                    
                    // Check if first time user
                    const seen = localStorage.getItem('agriverse_onboarded');
                    const savedMode = localStorage.getItem('agriverse_current_mode');
...
function updateProfileUI(profile) {
    ...
    const displayName = profile.full_name || localStorage.getItem('agriverse_fullname') || 'Người dùng';
    ...
    localStorage.setItem('agriverse_fullname', displayName);
    localStorage.setItem('agriverse_avatar', avatarUrl);
    syncHeaderProfile({ full_name: displayName, avatar_url: avatarUrl, current_level: profile.current_level, total_points: profile.total_points });
}

function syncHeaderProfile(profile = {}) {
    const name = profile.full_name || localStorage.getItem('agriverse_fullname') || 'Người dùng';
    const avatar = profile.avatar_url || localStorage.getItem('agriverse_avatar') || DEFAULT_AVATAR;
    const nameEl = $('profile-menu-name');
    const avatarEl = $('profile-menu-avatar');
    const badgeEl = $('profile-menu-badge');
    if (nameEl) nameEl.innerText = name;
    if (avatarEl) avatarEl.src = avatar;
    if (badgeEl) badgeEl.innerText = `Cấp ${profile.current_level || localStorage.getItem('agriverse_current_level') || 1}`;
}
...
function handleProfileMenuAction(action) {
    ...
    if (action === 'account') {
        showToast('Thông tin tài khoản sẽ được cập nhật sớm.', 'info');
    } else if (action === 'password') {
        showToast('Tính năng đổi mật khẩu đang được chuẩn bị.', 'info');
...
async function executeBlockAction(mode, action) {
    if (!selectedBlockId) { showToast("Chọn ô đất trước!", "warning"); return; }

    try {
        const res = await fetch(`${API_BASE}/block/${selectedBlockId}/action?user_id=${encodeURIComponent(USER_ID)}`, {
            method: "POST",
...
function saveBlockUserData() {
    ...
    updateTodoList();
    updateFarmSummary();
    showToast(`Đã lưu dữ liệu ô ${shortId}!`, 'success');
}
...
async function promptRenameKid() {
    ...
    showToast(`Đã đổi tên thành: ${name.trim()}!`, 'success');
}
...
function toggleRain() {
    ...
    if (parseInt(rainPct) >= 50) {
        ...
        showToast('🌧️ Trời có mưa theo dự báo KTTV! Hãy kiểm tra thoát nước ruộng.', 'warning');
        setTimeout(() => { document.body.style.background = ''; }, 3000);
    } else {
        showToast('☀️ Dự báo ít mưa — thời tiết thuận lợi cho công việc ngoài trời.', 'info');
    }
}
...
// ================================================================
// V0.0.7.0 REVOLUTION FEATURES (Merged)
// ================================================================
...
async function feedEcoPet() {
    ...
    setTimeout(() => {
        ...
        alert("Bé Rồng Đất rất thích độ ăn bé cho! ❤️ Độ thân thiết tăng lên.");
...
function handleRegisterRoleChange() {
    ...
    const parentGroup = $('parent-id-group');
    if (role === 'JUNIOR_ASSISTANT') {
        parentGroup.style.display = 'block';
    } else {
        parentGroup.style.display = 'none';
        $('register-parent-id').value = '';
    }
}
...
async function handleRegisterSubmit(event) {
    ...
    const role = $('register-role').value;
    const parentId = $('register-parent-id').value.trim();
...
async function handleLoginSubmit(event) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            localStorage.setItem('agriverse_user_id', data.user.user_id);
            localStorage.setItem('agriverse_username', data.user.username);
            localStorage.setItem('agriverse_fullname', data.user.full_name);
            localStorage.setItem('agriverse_role', data.user.role);
            localStorage.setItem('agriverse_farm_id', data.user.farm_id);
            
            // Set dynamic IDs
            USER_ID = data.user.user_id;
            FARM_ID = data.user.farm_id;
            
            showToast(`👋 Chào mừng bác ${data.user.full_name} đã quay trở lại ruộng vườn!`, 'success');
            
            // Hide auth screen and redirect
            ...
            // Reload page to re-initialize everything properly under the new user session
            setTimeout(() => {
...
function handleLogout() {
    if (confirm("Bác có chắc chắn muốn đăng xuất khỏi tài khoản không?")) {
        localStorage.removeItem('agriverse_token');
        localStorage.removeItem('agriverse_user_id');
        localStorage.removeItem('agriverse_username');
        localStorage.removeItem('agriverse_fullname');
        localStorage.removeItem('agriverse_role');
        localStorage.removeItem('agriverse_farm_id');
        localStorage.removeItem('agriverse_current_mode');
